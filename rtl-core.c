...
void fault_inject_trace(uint32_t f_index, uint32_t index, uint32_t reg_num,
                        ...
                        char *opcode) {
  printf(
      "[Fault Injector] f_index=%02d, index=%02d, reg_num=%02d, dependency=%d, "
      "opcode=%7s, size=%02d, value=",
      f_index, index, reg_num, dependency, opcode, size);

  int sz = size / 8;
  for (int i = 0; i < sz; i++) printf("%02X", value[sz - i - 1]);
...
major_type fault_inject_determine(major_type f_index, major_type index,
  ...
  if (drawing_lots == 1) return determine((double)1 / count_of_index);
  if (make_pre_table == 1) {
    check_count[index]++;
...
void fault_inject_set1(char *value) { *value |= (1 << get_fault_inject_pos()); }
...
void fault_inject(uint32_t f_index, uint32_t index, uint32_t reg_num,
  ...
  switch (injection_type) {
    case 0:
      fault_inject_flipbit(value);
      break;
    case 1:
      fault_inject_set0(value);
      break;
    case 2:
      fault_inject_set1(value);
      break;
  }
}
